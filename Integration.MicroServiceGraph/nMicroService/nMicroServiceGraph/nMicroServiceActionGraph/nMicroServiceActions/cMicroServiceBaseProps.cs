...
{
    public abstract class cMicroServiceBaseProps : IMicroServiceActionProps
    {
		public virtual string Host { get; set; }

		public cMicroServiceBaseProps()
		{
			foreach (PropertyDescriptor __Property in TypeDescriptor.GetProperties(this.GetType()))
			{
				if (typeof(cBaseEntity).IsAssignableFrom(__Property.PropertyType)
					|| typeof(IMappedEntity).IsAssignableFrom(__Property.PropertyType)
					|| typeof(IEntityList).IsAssignableFrom(__Property.PropertyType))
				{
		...
		public List<dynamic> ConvertToDynamicList(IList _List, Type _GenericType)
        {
                    ...
                    if (__IsVirtual != null && __IsVirtual.Value)
                    {
                        if (typeof(IList).IsAssignableFrom(__PropertyInfo.PropertyType))
                        {
                            ...
                            if (__Type.IsGenericType && __Type.GetGenericTypeDefinition() == typeof(List<>))
                            {
                                ...
                                ConvertToDynamicList((IList)__Object, __ItemType);
                            }
                            else
                            {
