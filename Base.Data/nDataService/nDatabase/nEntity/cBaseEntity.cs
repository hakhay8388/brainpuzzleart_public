...
namespace Base.Data.nDataService.nDatabase.nEntity
{
    public class cBaseEntity : cPropertyHookController
    {
        ...
        [DBField(_PrimaryKey: true, _KeyOrderNo: 1, _Nullable: false, _DataType: EDataType.Bigint, _DefaultValue: -1)]
        ...
        public void Refresh()
        {
            ...
            foreach (PropertyInfo __PropertyInfo in __PropertList)
            {
                ...
                {
                    DBField __DBField = __PropertyInfo.GetCustomAttribute<DBField>();
                    if (__DBField != null)
                    {
                        if (__Entity != null)
                        {
