...
namespace Base.Data.nDataService.nDatabase.nEntity.nEntityTable
{
    public class cEntityTable 
    {
        ...
        public void Reload()
        {
            ...
            foreach (PropertyInfo __PropertyInfo in __PropertList)
            {
                ...
                {
                    if (__PropertyInfo.GetCustomAttribute<DBField>() != null)
                    {
                        EntityFieldList.Add(new cEntityColumn(this, __PropertyInfo.Name, __PropertyInfo.PropertyType, __PropertyInfo.GetCustomAttribute<DBField>()));
                    }                    
                }                
            }
        }
                ...
                if (__IsVirtual != null && __IsVirtual.Value && !__PropertyInfo.PropertyType.IsPrimitiveWithString() && !__PropertyInfo.PropertyType.IsAssignableFrom(typeof(DateTime)))
                {
                    if (EntityManager.Database.GetEntityType().IsAssignableFrom(__PropertyInfo.PropertyType))
                    {
                        if (EntityType == __PropertyInfo.PropertyType)
                        {
                            ...
                            __EntityTable.EntityFieldList.Add(new cEntityColumn(__EntityTable, __PropertyInfo.Name, typeof(int), new DBField(_Nullable: true, _DataType: EDataType.Bigint, _ToForeingKey: EntityType)));
                        }
                        else
                        {
                            cEntityTable __EntityTable = EntityManager.GetEntityTableByEnitityType(__PropertyInfo.PropertyType);
                            __EntityTable.EntityFieldList.Add(new cEntityColumn(__EntityTable, TableForeing_ColumnName_For_InOtherTable, typeof(int), new DBField(_Nullable: false, _DataType: EDataType.Bigint, _DefaultValue: 0, _ToForeingKey: EntityType)));
                        }
                    }
                    else if (typeof(IEntityList).IsAssignableFrom(__PropertyInfo.PropertyType))
                    {
