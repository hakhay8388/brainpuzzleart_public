...
namespace Core.GenericWebScaffold.nWebGraph.nSessionManager
{
    public class cSessionManager : cCoreObject
    {
        ...
        public List<cSession> GetSessionByActorID(long _ActorID)
        {
            lock (SessionItems)
            {
                return SessionItems.Where(__Item => __Item.User != null && __Item.User.Actor.GetValue().ID == _ActorID).ToList();
            }
        }
        ...
        public List<cSession> GetSessionByActorID(List<long> _ActorIDList)
        {
            lock (SessionItems)
            {
                return SessionItems.Where(__Item => __Item.User != null && _ActorIDList.Contains(__Item.User.Actor.GetValue().ID)).ToList();
            }
        }
